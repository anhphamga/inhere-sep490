...
import { Eye, MessageSquare, ShieldX, RefreshCw } from 'lucide-react'
import {
...
export default function ReviewManagementPanel({ title = 'Quản lí đánh giá' }) {
  ...
  const [reasonModal, setReasonModal] = useState({ open: false, reviewId: '', reason: '' })
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      // Ignore stats errors to keep panel usable.
    }
  }, [])
  ...
  }, [hasReply, page, rating, search, sortBy, sortOrder])
  ...
  const submitHide = async () => {
    try {
      ...
      setReasonModal({ open: false, reviewId: '', reason: '' })
      await refreshAll()
      ...
      showToast(apiError?.response?.data?.message || 'Không thể ẩn đánh giá')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const statCards = useMemo(() => ([
    ...
    { label: 'Bị ẩn', value: Number(stats?.hiddenReviews || 0), tone: 'text-rose-600' },
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Quản lí đánh giá: lên ngay khi gửi, chỉ ẩn hoặc phản hồi.</p>
        </div>
        <button
          type="button"
          onClick={refreshAll}
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] border-collapse text-left text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td className="px-4 py-8 text-slate-500" colSpan={7}>Đang tải dữ liệu...</td></tr>
              ) : null}
              {!loading && rows.length === 0 ? (
                <tr><td className="px-4 py-8 text-slate-500" colSpan={7}>Không có đánh giá nào</td></tr>
              ) : null}
                ...
                <tr key={row._id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/60">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-wrap gap-1.5">
                      ...
                      <button type="button" onClick={() => setReasonModal({ open: true, reviewId: row._id, reason: '' })} className="rounded-md border border-slate-200 p-1.5 text-slate-700 hover:bg-slate-100" title="Ẩn đánh giá">
              ...
              <div className="space-y-4">
                ...
                {detail?.sellerReply?.content ? (
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-4">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-emerald-700">Phản hồi của cửa hàng</p>
      ...
      {reasonModal.open ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 px-4 py-6">
          <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-2xl">
            <h3 className="text-lg font-semibold text-slate-900">Ẩn đánh giá</h3>
            <p className="mt-1 text-sm text-slate-500">Nhập lý do ẩn đánh giá (không bắt buộc).</p>
            <textarea
              rows={4}
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button type="button" onClick={() => setReasonModal({ open: false, reviewId: '', reason: '' })} className="rounded-lg border border-slate-300 px-4 py-2 text-sm text-slate-700 hover:bg-slate-50">
                Hủy
              </button>
              <button type="button" disabled={submitting} onClick={submitHide} className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 disabled:opacity-60">
